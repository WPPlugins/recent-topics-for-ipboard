 ...
 * Plugin URI:  http://thekrotek.com/wordpress-extensions/ipboard-recent
				...
				if ($args['widget'] || ($this->params['mode'] != 'widget')) {		
					...
					if ($this->getOption('dbname') && $this->getOption('dbhost') && $this->getOption('dbusername') && $this->getOption('dbpassword')) {
						...
						if ($ipbdb) {
							...
							$query .= "WHERE t.forum_id".($forums ? ($this->getOption('action', 'exclude') == "exclude" ? " NOT IN" : " IN")." (".implode(', ', $forums).")" : "");
							$query .= (!$this->getOption('closed', '0') ? " AND t.state = 'open' AND t.approved > 0" : "")." ";
							$query .= ($this->getOption('empty', '0') ? " AND t.posts <= 1 " : "");
							$query .= "ORDER BY t.last_post DESC LIMIT 0, ".$this->getOption('topics', '5');
							...
							foreach ($topics as $topic) {
								...
								$query .= "WHERE p.topic_id = ".esc_attr($topic['topic_id'])." ORDER BY p.pid ".($postmode == 'first' ? "ASC" : "DESC")." LIMIT 0, 1";
					...
					$output .= "Powered by <a href='http://thekrotek.com' target='_blank' title='Discover the truth!'>The Krotek</a>";
					$output .= "</div>";
					$output .= "</div>";
				}
			} else {
